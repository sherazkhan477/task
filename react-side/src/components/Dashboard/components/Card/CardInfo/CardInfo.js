    ...
    <Modal onClose={props.onClose}>
      <div className="p-8 flex flex-col gap-8 min-w-[550px] w-fit max-w-[650px] h-fit">
        ...
        <div className="w-full flex flex-col gap-4">
          ...
          <input
            type="date"
            className="w-fit border-2 border-gray-300 rounded-md text-base p-2"
        ...
        <div className="w-full flex flex-col gap-4">
          ...
          <div className="flex gap-2 flex-wrap">
            {values.labels?.map((item, index) => (
              <label
                key={index}
                className="rounded-full bg-gray-500 text-white px-2 py-1 flex items-center gap-1"
              >
                {item.text}
          ...
          <ul className="flex gap-4 ml-5">
            {colors.map((item, index) => (
              <li
                key={index}
                className={`w-5 h-5 rounded-full cursor-pointer ${selectedColor === item ? "shadow-[0_0_0_3px_yellow]" : ""}`}
          ...
          <div className="flex flex-col gap-4">
            {values.tasks?.map((item) => (
              <div key={item.id} className="flex gap-2">
                ...
                <Trash className="h-4 w-4 cursor-pointer" onClick={() => removeTask(item.id)} />
              </div>
            ))}
          </div>
          <Editable
